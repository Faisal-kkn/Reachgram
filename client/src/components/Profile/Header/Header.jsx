...
function Header({ data, handleSubmit, editbutton }) {
    ...
    const submitProfile = () => {
        ...
        axios.put('http://localhost:5000/edit_profile', formData, {
            ...
            console.log('response.dataaaaaaaaaaa');
            console.log(response.data);
            if (!response.data.status) setEditProfileErr(response.data)
        })
    }
    useEffect(() => {
        ProfileData()
        ...
        <>
            <div className='mx-auto max-w-7xl flex justify-between flex-col md:flex-row  gap-3 w-12/12 bg-[#314f5f6e] rounded-[10px] p-[15px] text-white items-center'>
                <div className='flex gap-5 items-center w-full lg:w-4/12'>
                    <div className='border-[3px] rounded-[10px] min-w-[100px] h-[100px] bg-cover' style={{ backgroundImage: `url(${userDetails.profile || 'https://images.unsplash.com/photo-1485893086445-ed75865251e0?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=580&q=80'})` }}></div>
                    <div className=''>
                        ...
                        <p className=' font-light text-[#a8c2d1] block pt-1' style={{ 'display': '-webkit-box', '-webkit-line-clamp': '2', '-webkit-box-orient': 'vertical', 'overflow': 'hidden' }}>{userDetails?.about || 'write your about'}</p>
                    </div>
                </div>
